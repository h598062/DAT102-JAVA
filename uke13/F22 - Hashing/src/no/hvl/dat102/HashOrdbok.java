...
public class HashOrdbok<K, V> implements OrdbokInterface<K, V> {
	...
	private static final int STANDARD_KAPASITET = 17; // Bør vere primtall (og aldri et jevnt tall)
	...
	public HashOrdbok(int startKapasitet) {
		...
		antallElement = 0;
		tabellStorrelse = finnNestPrimtall(startKapasitet);
	...
	private boolean erPrimtall(int t) {
		...
		/* 
		 * Nå er t et oddetall > 2. Det betyr at om det ikke er et primtall må
		 * det vær et produkt av minst to oddetall. Et av faktorene må være <=
		 * kvadratroten av tallet. To faktorer som er større enn kvadratroten av
		 * tallet blir større enn t.
		 * 
		 * Dermed er det tilstrekkelig å vise at t har ingen faktorer som er <=
		 * enn kvadratroten av tallet for å vise at det er et primtall.
		 */
	...
	private int finnNestPrimtall(int t) {
		int p = t;
